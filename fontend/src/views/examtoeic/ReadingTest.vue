  ...
  <div class="d-flex flex-column min-vh-100">
    <main class="flex-grow-1 container mt-5">
      <div class="text-center mb-4">
        <h3 class="page-header text-primary fw-bold">Thi Thử Đọc</h3>
      ...
      <div v-if="questions.length" class="quiz-container">
        <div v-for="(question, index) in questions" :key="question.questionreadingid" class="quiz-card">
          <div class="card shadow-sm h-100">
            <div class="card-body">
              <h5 class="card-title text-primary fw-bold">
                {{ question.questionreadingscript }}
              </h5>
              <h5 class="card-title text-primary fw-bold">
                Câu {{ index + 1 }}: {{ question.questionreadingask }}
              </h5>
              ...
              <div v-if="submitted" class="mt-3">
                <p>
                  <span :class="{'text-success': answers[index] === question.questionreadinganswercorrect, 'text-danger': answers[index] !== question.questionreadinganswercorrect}">
                    Bạn đã trả lời: {{ answers[index] || 'Chưa trả lời' }}
                  </span>
                </p>
                <p class="text-muted">
                  Đáp án đúng: <strong>{{ question.questionreadinganswercorrect }}</strong>
                </p>
                <p class="text-info">
                  Giải thích: {{ question.questionreadingexplain || 'Không có giải thích.' }}
                </p>
              </div>
            </div>
          </div>
...
// Quay về trang chủ
const goHome = () => {
  window.location.href = "/listreadingtest";
};
...
const loadQuestions = async () => {
  try {
    ...
    const { data } = await axios.get(`${baseUrl}/api/admin/reading/loadQuestionReading`, {
      params: { readingid },
    });
    questions.value = data.map((q) => ({
      ...q,
      answers: [
        q.questionreadinganswer1,
        q.questionreadinganswer2,
        q.questionreadinganswer3,
        q.questionreadinganswer4,
      ],
      questionreadingexplain: q.questionreadingexplain, // Giải thích
    }));
...
const submitQuiz = () => {
  ...
  score.value = questions.value.reduce((sum, question, index) => {
    return sum + (answers.value[index] === question.questionreadinganswercorrect ? 1 : 0);
  }, 0);
